...
class Interpreter {
    ...
    private void visitClassStmt(ClassStmt classStmt) {
        currentEnvironment.define(classStmt.name().lexeme(), null);

        Map<String, LoxFunction> methods = new HashMap<>();
        for (Function method : classStmt.methods()) {
            LoxFunction function = new LoxFunction(method, currentEnvironment, method.name().lexeme().equals("init"));
            methods.put(method.name().lexeme(), function);
        }

        LoxClass loxClass = new LoxClass(classStmt.name().lexeme(), methods);
        currentEnvironment.assign(classStmt.name(), loxClass);
    }

    private void visitFunctionStmt(Function functionStmt) {
        LoxFunction loxFunction = new LoxFunction(functionStmt, currentEnvironment, false);
    ...
    private Object evaluate(Expr expr) {
        return switch (expr) {
            ...
            case Get get -> visitGetExpr(get);
            case Set set -> visitSetExpr(set);
            case This thisExpr -> visitThisExpr(thisExpr);
        };
    }

    private Object visitGetExpr(Get getExpr) {
    ...
    private Object visitSetExpr(Set setExpr) {
    ...
    private Object visitThisExpr(This thisExpr) {
